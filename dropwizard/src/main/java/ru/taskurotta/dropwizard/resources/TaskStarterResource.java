...
public class TaskStarterResource {
    ...
    private TaskContainer injectIdsIfAbsent(TaskContainer target) {
        ...
        return new TaskContainer(taskId, processId, target.getMethod(), target.getActorId(), target.getType(), target.getStartTime(), target.getNumberOfAttempts(), target.getArgs(), target.getOptions(), target.getFailTypes());
    }
