...
public class GCService {
    ...
    private static Logger logger = LoggerFactory.getLogger(GCService.class);
    ...
    private void init() {
        executorService = Executors.newFixedThreadPool(threadCount, new ThreadFactory() {
            ...
            public Thread newThread(Runnable r) {
                ...
                thread.setName("GarbageCollector-" + counter++);
                return thread;
            }
        });
        ...
        if(startupRecovery) {
            ...
            logger.info("[{}] processes submitted to garbage collector", result);
        }
    }
