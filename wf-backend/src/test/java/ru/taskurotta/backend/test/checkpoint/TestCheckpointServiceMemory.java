...
public class TestCheckpointServiceMemory {
    ...
    public void testCheckpointServiceMemory() {
        ...
        Checkpoint c1 = new Checkpoint(uuid1, type1, time1);
        Checkpoint c2 = new Checkpoint(uuid2, type1, time2);
        Checkpoint c3 = new Checkpoint(uuid3, type2, time3);

        checkpointService.addCheckpoint(c1);
        checkpointService.addCheckpoint(c2);
        checkpointService.addCheckpoint(c3);

        CheckpointQuery query1 = new CheckpointQuery();
        query1.setMaxTime(time3 + 100l);
        query1.setType(type1);
        query1.setMinTime(time1 - 100);

        CheckpointQuery query2 = new CheckpointQuery();
        query2.setMaxTime(time3 + 100l);
        query2.setType(type2);
        ...
        checkpointService.removeCheckpoint(new Checkpoint(c1.getGuid(), c1.getType(), c1.getTime()));
