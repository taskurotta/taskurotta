...
public class QueryResultIterator implements Cursor {
    ...
    public static ThreadLocal<Boolean> isDelayQueueRequest = new ThreadLocal<>();
    ...
    private void init(int flags, long cursorId, int size, Iterator<DBObject> iterator){
        ...
        if (isDelayQueueRequest.get() != null && size > 0) {
            System.err.println("\t\t\t\t\t\t DELAY QUEUE RESPONSE SIZE = " + size);
        }
        _curSize = size;
        _cur = iterator;
        _sizes.add(size);
