...
public abstract class AbstractSerializationService implements SerializationService {
    ...
    protected static final org.slf4j.Logger myLogger = LoggerFactory.getLogger(AbstractSerializationService.class);
    private ConcurrentHashMap<Class, SerializerAdapter> serializersCache = new ConcurrentHashMap<>();

    protected final SerializerAdapter serializerFor(final Object object) {
        ...
        serializer = serializerForInternal(object);
        if (serializer == null) {
            throw new IllegalStateException("Unknown serializer for class " + type);
        }

        if (serializersCache.putIfAbsent(type, serializer) == null && myLogger.isDebugEnabled()) {
            myLogger.debug("Cached new serializer: {}", serializer.getImpl().getClass().getName());
        }

        return serializer;
    }

    protected final SerializerAdapter serializerForInternal(Object object) {
        /*
