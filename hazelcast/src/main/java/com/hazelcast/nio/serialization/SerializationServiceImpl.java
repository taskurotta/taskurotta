...
public class SerializationServiceImpl implements SerializationService {
    ...
    protected final SerializerAdapter serializerFor(final Class type) {
        ...
        if (serializersCache.putIfAbsent(type, serializer) == null && logger.isDebugEnabled()) {
            logger.debug("Cached new serializer: {}", serializer.getImpl().getClass().getName());
        }

        return serializer;
    }

    private SerializerAdapter serializerForInternal(final Class type) {        SerializerAdapter serializer;
