...
public class EnqueueTaskJob implements Job {
    ...
    public static TaskContainer renewTaskGuids(TaskContainer target) {
        ...
        return new TaskContainer(newGuid, newGuid, target.getMethod(), target.getActorId(), target.getType(), target.getStartTime(), target.getErrorAttempts(), target.getArgs(), target.getOptions(), target.isUnsafe(), target.getFailTypes());
    }
